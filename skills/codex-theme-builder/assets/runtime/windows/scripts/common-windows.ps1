...
function Initialize-DreamSkinApplicationActivationManager {
  if ('DreamSkin.Windows.ApplicationActivationManager' -as [type]) { return }
  Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;

namespace DreamSkin.Windows {
  [Flags]
  ...
  public static class PackagedAppActivator {
    ...
    public static uint Activate(string appUserModelId, string arguments) {
      var manager = (IApplicationActivationManager)new ApplicationActivationManager();
      uint processId;
      int hresult = manager.ActivateApplication(
        appUserModelId,
...
function Start-DreamSkinPackagedCodex {
  param(
  ...
  return [DreamSkin.Windows.PackagedAppActivator]::Activate($appUserModelId, $arguments)
}
