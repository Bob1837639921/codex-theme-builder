...
Use the supplied reference as the visual truth. When the user supplies an existing Codex screenshot plus explicit change requests, preserve the native layout and generate one constrained implementation-ready preview if a preview is useful. When no visual target exists and the requested direction materially branches, follow [visual-direction-gate.md](references/visual-direction-gate.md): generate exactly three conversation-page previews first, wait for a selection, and defer home, usage, selected-thread, composer-edge, motion, and packaging assets until the direction is approved. If the user explicitly delegates the visual choice or requests an uninterrupted fully automatic run, score those three options for requirement fit, native-control compatibility, feasibility, readability, and runtime cost; record the decision and continue with the highest-scoring option without pausing.
...
Read [theme-contract.md](references/theme-contract.md) and [regression-invariants.md](references/regression-invariants.md) before authoring or modifying a theme. The latter is the mandatory shared record of verified defects that must not return in a new palette or artwork direction. When creating or porting a visual direction, first read [visual-direction-gate.md](references/visual-direction-gate.md), then read [new-theme-blueprint.md](references/new-theme-blueprint.md) and [conversation-quality-baseline.md](references/conversation-quality-baseline.md); together they define the approval gate, reusable surface map, and soft/full conversation quality bar that every new theme must meet. Read [artwork-quality.md](references/artwork-quality.md) before creating, replacing, enlarging, or compressing full-canvas artwork. Read [runtime-architecture.md](references/runtime-architecture.md) before modifying shared runtime, launcher, process-control, progress, or switching infrastructure. Read [windows-runtime.md](references/windows-runtime.md) before live application. Use [qa-checklist.md](references/qa-checklist.md) for final verification.
...
powershell -ExecutionPolicy Bypass -File scripts/new-theme.ps1 `
  ...
  -PreviewImage "C:\path\preview-320x180.webp" `
...
The four video arguments are optional. The dedicated 320x180 switcher preview and selected-thread background are required for every newly generated theme. Keep the preview at or below 256 KB so the metadata-first catalog can load visible cards without decoding full-canvas artwork. Use a transparent stretch-safe selected strip with a quiet text center, not a full-scene crop or a late-loading label icon. When motion is approved, keep home and conversation clips independent in both soft and full tiers so route changes and motion-level changes can use the shared atomic handoff without falling back through a mismatched static composition.
...
6. Work through every row in `new-theme-blueprint.md` and every applicable invariant in `regression-invariants.md`, including both compact and full usage surfaces. Keep generic DOM discovery and safety behavior in the shared runtime; keep palette, imagery, spacing, and theme identity in the theme folder.
