...
The desktop launcher supports a true cold start. It uses one branded WinForms flow instead of consecutive system popups: a restart confirmation only when Codex is already running, followed by a single progress window that becomes a brief success state and closes automatically. Progress is determinate and event-driven: theme validation, runtime discovery, loopback-port allocation, Codex activation, CDP readiness, injector startup, state persistence, and final injection verification each report their own milestone. The window reaches 100% only after verification succeeds. The success state reports only the current launch result and never tells users to click the shortcut again. The launcher then brings the verified Codex main window to the foreground. Diagnostic milestones are written to `%LOCALAPPDATA%\CodexDreamSkinV2\desktop-launch.log`.
