...
- For every theme with scene video, test route handoff in both directions after a
  populated conversation has been mounted. A shell replacement must keep the
  outgoing video visible, temporarily relocate it to the stable document canvas
  when necessary, and reattach it without exposing the static poster.
