...
The direct native content toolbar `main.main-surface > header.app-header-tint` is part of the themed scene, not an opaque card. The shared runtime enforces a five-percent theme tint across home and conversation routes so the matching artwork or video remains clearly visible beneath it. Do not apply backdrop blur to either the native header or its isolated paint child: blur can turn bright artwork into a wide gray/white band and adds avoidable repaint cost. Every bundled or newly generated theme must explicitly declare `--theme-toolbar-surface` as its own palette tint; do not rely on the generic surface fallback, because that makes multiple themes look unadapted. Prefer a solid six-digit color and let the shared `.dream-toolbar-glass` layer own the transparency so alpha is not compounded. Theme CSS may set readable foreground, icon, and hover colors, but must not restore an opaque fill, decorative header pseudo-element, hard divider, blur, second raster crop, fixed height, or altered native control geometry. Never fake toolbar continuity by painting a 36–48px opaque strip into `body`; the route-matching artwork itself must continue beneath the toolbar. The Windows/Electron menu strip above this toolbar remains system-owned.
...
Codex's native `.app-shell-main-content-top-fade` / hashed `_MainContentTopFade_` is a light-mode gradient that can paint an opaque 46px white band on both home and conversation routes. The shared runtime disables it for every active theme; do not restore it in theme CSS. Use the shared five-percent toolbar tint and route artwork for continuity instead.
