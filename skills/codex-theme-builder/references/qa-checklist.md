...
- If `backgroundVideo` is present, verify it appears only at the full motion tier, is muted/looping/non-interactive, reveals the static poster until playable, pauses while the document is hidden, is removed outside the full tier, and never runs under reduced motion.
